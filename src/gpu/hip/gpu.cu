        ...
        int *bin_locator, int count, double DMCutoff, double XCCutoff, int nbins)
{
    ...
    gpuMemcpy(gpweight, d_gpweight, gpu->gpu_xcq->npoints * sizeof(unsigned char), hipMemcpyDeviceToHost);
    gpuMemcpy(cfweight, d_cfweight, nbins * gpu->nbasis * sizeof(unsigned int), hipMemcpyDeviceToHost);
    gpuMemcpy(pfweight, d_pfweight, nbins * gpu->nbasis * gpu->gpu_basis->maxcontract * sizeof(unsigned int), hipMemcpyDeviceToHost);
    ...
    if (incoreInt) {
        ...
        for (int i = 0; i<totalBuffer; i++) {
            ...
            if (currentInt >= availableERI) {
                ...
                //gpuMemcpy(gpu->aoint_buffer[0]->_devData, gpu->aoint_buffer[0]->_hostData, currentInt*sizeof(ERI_entry), hipMemcpyHostToDevice);
                gpuMemcpy(gpu->aoint_buffer[0]->_devData, intERIEntry + startingInt, currentInt*sizeof(ERI_entry), hipMemcpyHostToDevice);
        ...
        for ( int i = 0; i < bufferPackNum; i++) {
            ...
            for (int j = 0; j < thisBuffer; j++) {
                ...
                if (bufferIndex >= availableERI) {
                    //gpuMemcpyAsync(gpu->aoint_buffer[0]->_hostData, gpu->aoint_buffer[0]->_devData, bufferIndex*sizeof(ERI_entry), hipMemcpyHostToDevice, stream[0]);
                    gpu->aoint_buffer[0]->Upload();
    ...
    for (int iBatch = 0; iBatch < iBatchCount; iBatch = iBatch + streamNum) {
        ...
        for (int i = 0; i < streamNum && iBatch + i < iBatchCount; i++) {
            gpuMemcpyAsync(&gpu->intCount->_hostData[i], &gpu->intCount->_devData[i],
                    sizeof(QUICKULL), hipMemcpyDeviceToHost, stream[i]);
            gpuMemcpyAsync(gpu->aoint_buffer[i]->_hostData, gpu->aoint_buffer[i]->_devData,
                    sizeof(ERI_entry) * gpu->intCount->_hostData[i], hipMemcpyDeviceToHost, stream[i]);
        }
